...
def main():
    ...
    if not execute_sql_file(
        conn, "migrations/add_funding_constraints.sql", "Adicionando colunas, constraints e índices"
    ):
    ...
    try:
        ...
        for row in rows:
            ...
            try:
                ...
                if org_id:
                    cur.execute(
                        "UPDATE sofia.funding_rounds SET organization_id = %s WHERE id = %s", (org_id, funding_id)
                    )
                    org_added += 1
            except:
                pass
    ...
    if not execute_sql_file(conn, "migrations/deduplicate-funding.sql", "Removendo registros duplicados"):
