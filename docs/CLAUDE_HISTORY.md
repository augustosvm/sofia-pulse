...
## 🗄️ ESTRUTURA DE TABELAS UNIFICADAS (22 Dez 2025)

**REGRA CRÍTICA**: Todas as entidades do mesmo tipo devem estar na mesma tabela, diferenciadas por campo `type`.

### **Pessoas Físicas** (`sofia.persons`)

**REGRA**: TODAS as pessoas físicas devem estar nesta tabela, NUNCA criar tabelas separadas.

**Estrutura**:
```sql
CREATE TABLE sofia.persons (
  id SERIAL PRIMARY KEY,
  full_name TEXT NOT NULL,
  normalized_name TEXT UNIQUE,  -- LOWER(TRIM(REGEXP_REPLACE(...)))
  type VARCHAR(50) NOT NULL,     -- 'researcher', 'author', 'founder', 'ceo', etc.
  ...
  country VARCHAR(100),
  city VARCHAR(100),
  primary_affiliation TEXT,      -- Universidade/empresa principal
  
  -- Metadata
  data_sources TEXT[],           -- ['openalex', 'arxiv', 'manual']
  metadata JSONB,                -- Campos específicos por tipo
...
CREATE INDEX idx_persons_type ON sofia.persons(type);
CREATE INDEX idx_persons_normalized_name ON sofia.persons(normalized_name);
```
...
**Estatísticas Atuais** (22 Dez 2025):
- **222,454 pessoas** consolidadas:
  - `researcher`: 130,934 (pesquisadores de universidades - OpenAlex)
  - `author`: 91,520 (autores de papers - ArXiv, OpenAlex)

**Como Inserir**:
```typescript
import { PersonsInserter } from './shared/persons-inserter';

await personsInserter.insertPerson({
...
**Relacionamentos**:
- `person_papers` - Liga pessoas a papers (many-to-many)
- `person_organizations` - Liga pessoas a organizações (many-to-many)

---

### **Pessoas Jurídicas** (`sofia.organizations`)

**REGRA**: TODAS as organizações devem estar nesta tabela, NUNCA criar tabelas separadas.

**Estrutura**:
```sql
CREATE TABLE sofia.organizations (
  id SERIAL PRIMARY KEY,
  org_id VARCHAR(255) NOT NULL,
  name TEXT NOT NULL,
  normalized_name TEXT,
  type VARCHAR(50) NOT NULL,     -- 'university', 'ngo', 'startup', 'vc_firm', etc.
  source VARCHAR(100) NOT NULL,  -- 'openalex', 'crunchbase', 'manual'
  ...
  CONSTRAINT organizations_unique UNIQUE (org_id, source)
);

CREATE INDEX idx_organizations_type ON sofia.organizations(type);
CREATE INDEX idx_organizations_source ON sofia.organizations(source);
CREATE INDEX idx_organizations_country ON sofia.organizations(country);
```

**Tipos Permitidos**:
- `university` - Universidades
- `research_center` - Centros de pesquisa
- `ngo` - ONGs
- `startup` - Startups
- `ai_company` - Empresas de IA
- `vc_firm` - Fundos de venture capital
- `government_agency` - Agências governamentais
- `corporation` - Corporações

**Estatísticas Atuais**:
- **418 organizações** consolidadas (dados de OpenAlex, AI Companies, NGOs)

**Como Inserir**:
```typescript
import { OrganizationsInserter } from './shared/organizations-inserter';

await organizationsInserter.insertOrganization({
  org_id: 'I136199984',
...
**Relacionamentos**:
- `person_organizations` - Liga organizações a pessoas (many-to-many)
- `funding_rounds` - Liga startups a rounds de investimento

---

### **Research Papers** (`sofia.research_papers`)

**REGRA**: TODOS os papers devem estar nesta tabela, diferenciados por `source`.

**Estrutura**:
```sql
CREATE TABLE sofia.research_papers (
  id SERIAL PRIMARY KEY,
  title TEXT NOT NULL,
  source VARCHAR(50) NOT NULL,     -- 'arxiv', 'openalex', 'bdtd', 'nih'
  source_id VARCHAR(255) NOT NULL,
  
  abstract TEXT,
  authors TEXT[],                  -- Array de nomes (será linkado a persons)
  keywords TEXT[],
...
**Fontes**:
- `arxiv` - ArXiv AI/ML Papers
- `openalex` - OpenAlex Research Papers
- `bdtd` - Brazilian Theses
- `nih` - NIH Grants

**Total**: 7,104 papers consolidados

---

### **⚠️ PROCEDIMENTO OBRIGATÓRIO PARA NOVOS COLETORES**

**Ao criar um novo collector de pessoas físicas**:
1. ✅ **SEMPRE** inserir em `sofia.persons` (NUNCA criar nova tabela)
2. ✅ Usar `PersonsInserter` do `scripts/shared/persons-inserter.ts`
3. ✅ Definir `type` correto (researcher, author, founder, etc.)
4. ✅ Normalizar nomes com `LOWER(TRIM(REGEXP_REPLACE(...)))`
5. ✅ Usar ON CONFLICT para evitar duplicatas

**Ao criar um novo collector de organizações**:
1. ✅ **SEMPRE** inserir em `sofia.organizations` (NUNCA criar nova tabela)
2. ✅ Usar `OrganizationsInserter` do `scripts/shared/organizations-inserter.ts`
3. ✅ Definir `type` correto (university, startup, ngo, etc.)
4. ✅ Usar `(org_id, source)` como unique key
5. ✅ Armazenar dados específicos em `metadata` JSONB

**Ao criar um novo collector de papers**:
1. ✅ **SEMPRE** inserir em `sofia.research_papers` (NUNCA criar nova tabela)
2. ✅ Usar `ResearchPapersInserter` do `scripts/shared/research-papers-inserter.ts`
3. ✅ Definir `source` correto (arxiv, openalex, etc.)
4. ✅ Extrair autores e inserir em `sofia.persons` com type='author'
5. ✅ Criar links em `person_papers` para relacionamento

**Exemplo Completo - Collector de Papers**:
```typescript
import { ResearchPapersInserter } from './shared/research-papers-inserter';
import { PersonsInserter } from './shared/persons-inserter';

// 1. Inserir paper
await papersInserter.insertPaper({
...
// 3. Criar links em person_papers (automático via trigger ou manualmente)
```
...
✅ **Sem Duplicatas**: Deduplicação automática via UNIQUE constraints
✅ **Escalável**: Adicionar novos tipos sem alterar schema
✅ **Compliance**: Todos os dados em tabelas auditáveis
✅ **Relacionamentos**: Links corretos entre entidades
✅ **Analytics**: Queries cross-database funcionam corretamente
✅ **Zero Data Loss**: ON CONFLICT preserva dados existentes
...
##  ATUALIZA��O: 2025-12-23

###  1. Agendador Inteligente (Windows)
**Problema**: Cron n�o roda nativamente no Windows.
**Solu��o**: Criado intelligent_scheduler.py que substitui o Crontab, rodando scripts Python e TS/Node com intervalos configur�veis, retentativas e alertas no WhatsApp.
...
- **MDIC**: Usa mapeamento de estados pr�prio (STATE_MAP), n�o usa o normalizador compartilhado.
- **FIESP**: Foca em �ndices agregados (INA/Sensor), sem granularidade de cidades que exija normaliza��o complexa.
- **A��o**: Mantidos como est�o por hora, pois funcionam. Normaliza��o centralizada sugerida para futuro.
