...
class DatabaseInventory:
    ...
    def get_constraints(self, schema: str, table: str) -> List[Dict]:
        ...
        return [{'name': name, 'definition': clause} 
                for name, clause in self.cursor.fetchall()]
    ...
    def find_issues(self):
        ...
        for schema, data in self.inventory['schemas'].items():
            for table_name, table_info in data.get('tables', {}).items():
                ...
                non_pk_indexes = [idx for idx in table_info.get('indexes', []) 
                                 if not idx.get('primary')]
                ...
                has_timestamp = any(col['name'] in ['created_at', 'updated_at'] 
        ...
        with open(filename, 'w') as f:
            ...
            for schema, data in self.inventory['schemas'].items():
                ...
                for table_name, table_info in data.get('tables', {}).items():
                    ...
                    f.write(f"| {table_name} | {table_info['row_count']:,} | {table_info['size']} | {pk} | {idx_count} | {table_info['last_updated']} |\n")
