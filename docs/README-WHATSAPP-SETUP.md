...
**Causa provável**: O número pessoal (+55 XX XXXXX-XXXX) precisa estar **pré-autorizado** no WhatsApp Business API.

**Como funciona o WhatsApp Business API:**
1. Você cria uma conta Business com o número +55 XX XXXXX-XXXX (Business)
2. Você REGISTRA quais números podem RECEBER mensagens
3. Só números registrados recebem mensagens da API

**Solução:**
- Verificar no painel do WhatsApp Business se +55 XX XXXXX-XXXX está autorizado
...
Isso configura:
```env
WHATSAPP_NUMBER=YOUR_WHATSAPP_NUMBER       # Recipient (seu número)
WHATSAPP_SENDER=YOUR_BUSINESS_NUMBER        # Sender (WhatsApp Business)
...
**Problema**: Número +55 XX XXXXX-XXXX não está registrado no WhatsApp Business

**Solução**:
1. Acessar painel do WhatsApp Business API
2. Adicionar +55 XX XXXXX-XXXX à lista de números autorizados
...
- [ ] WhatsApp Business configurado (+55 XX XXXXX-XXXX (Business))
- [ ] Número pessoal autorizado (+55 XX XXXXX-XXXX)
...
POST http://localhost:8001/api/v2/chat
    payload: {
        query: "mensagem",
        phone: "YOUR_WHATSAPP_NUMBER"  ← Seu número (RECIPIENT)
    }
    ↓
sofia-mastra-rag processa
    ↓
WhatsApp Business API
    from: +55 XX XXXXX-XXXX (Business)  ← Business (SENDER)
    to: +55 XX XXXXX-XXXX   ← Você (RECIPIENT)
    ↓
...
**Recipient (Você)**: +55 XX XXXXX-XXXX
**Sender (Business)**: +55 XX XXXXX-XXXX (Business)
