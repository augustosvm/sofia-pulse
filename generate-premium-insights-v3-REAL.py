...
# ============================================================================
# CONTEXTO MACROECONÔMICO (v3.0)
# ============================================================================

import requests

def get_macro_context():
    """Coleta contexto macro: dólar BCB, Selic, Fed"""
    context = {}
    ...
    context['fed_rate'] = 5.25  # Atualizar manualmente conforme necessário

    return context

print("\n🌍 Coletando contexto macroeconômico...")
MACRO_CONTEXT = get_macro_context()
print(f"   💵 Dólar: R$ {MACRO_CONTEXT['dolar']:.2f}")
print(f"   📈 Selic: {MACRO_CONTEXT['selic']:.2f}%")
print(f"   🏦 Fed: {MACRO_CONTEXT['fed_rate']:.2f}%")
...
# Funding Rounds (COM país + comparação temporal v3.0)
try:
    ...
    # Comparação temporal (v3.0)
    if not df_funding.empty and 'announced_date' in df_funding.columns:
        df_funding['announced_date'] = pd.to_datetime(df_funding['announced_date'])
        now = pd.Timestamp.now()
        funding_30d = df_funding[df_funding['announced_date'] >= (now - pd.Timedelta(days=30))]
        funding_90d = df_funding[df_funding['announced_date'] >= (now - pd.Timedelta(days=90))]

        total_30d = funding_30d['amount_usd'].sum()
        total_90d = funding_90d['amount_usd'].sum()

        FUNDING_GROWTH = ((total_30d * 3 / total_90d) - 1) * 100 if total_90d > 0 else 0
        print(f"      📊 30d: ${total_30d/1e9:.1f}B | 90d: ${total_90d/1e9:.1f}B | Growth: {FUNDING_GROWTH:+.1f}%")
    else:
        FUNDING_GROWTH = 0
except Exception as e:
...
# B3 e NASDAQ (com detecção de anomalias v3.0)
try:
    df_b3 = pd.read_sql('''
        SELECT ticker, company, sector, price, change_pct, volume, collected_at
    ...
    # Detecção de anomalias (v3.0)
    if not df_b3.empty and 'ticker' in df_b3.columns:
        # Pegar apenas última coleta de cada ticker
        df_b3_latest = df_b3.drop_duplicates(subset='ticker', keep='first')

        # Calcular média 7d por ticker
        df_b3['collected_at'] = pd.to_datetime(df_b3['collected_at'])
        avg_7d = df_b3.groupby('ticker')['change_pct'].mean().to_dict()

        # Detectar anomalias: variação > 2% da média
        B3_ANOMALIES = []
        for _, row in df_b3_latest.iterrows():
            ticker = row['ticker']
            current_change = row['change_pct']
            avg_change = avg_7d.get(ticker, 0)

            if abs(current_change - avg_change) > 2:
                B3_ANOMALIES.append({
                    'ticker': ticker,
                    'company': row['company'],
                    'current': current_change,
                    'avg_7d': avg_change,
                    'delta': current_change - avg_change
                })

        if B3_ANOMALIES:
            print(f"      🚨 Anomalias detectadas: {len(B3_ANOMALIES)} ações")
    else:
        B3_ANOMALIES = []
...
if GEMINI_API_KEY:
    ...
    if available_models:
        ...
        funding_summary = f"{len(df_funding)} rodadas" if not df_funding.empty else "sem dados"
        anomalies_summary = f"{len(B3_ANOMALIES)} anomalias" if B3_ANOMALIES else "sem anomalias"

        prompt = f"""
Você é analista sênior de tecnologia e mercados globais da Sofia Pulse. Gere insight analítico PROFUNDO (não resumo descritivo).

**DADOS GEO-LOCALIZADOS**:
{all_insights_text}

**CONTEXTO MACRO** (v3.0):
- Dólar: R$ {MACRO_CONTEXT['dolar']:.2f}
- Selic: {MACRO_CONTEXT['selic']:.2f}%
- Fed: {MACRO_CONTEXT['fed_rate']:.2f}%

**COMPARAÇÃO TEMPORAL**:
- Funding growth (30d vs 90d): {FUNDING_GROWTH:+.1f}%
- Funding: {funding_summary}
- Anomalias B3: {anomalies_summary}

**TAREFA**: Escreva análise em 3 parágrafos seguindo estrutura:

1. **CAUSA** (Por que está acontecendo?)
   - Correlacione dados geo-localizados + macro + variação temporal
   - Identifique drivers principais (ex: tensões geopolíticas, taxas de juros, etc)
   - Use números concretos

2. **CONSEQUÊNCIA** (O que isso significa?)
   - Impacto para mercado BR e global
   - Setores/regiões favorecidos vs prejudicados
   - Padrões emergentes (ex: capital migrando de X para Y)

3. **OPORTUNIDADE** (Onde investir?)
   - Ações/setores/regiões concretas
   - Janelas de tempo (curto vs médio prazo)
   - Riscos a monitorar

**TOM**: Sagaz, cético, direto. Sem enrolação. Foco em ALPHA (insights não-óbvios que outros não têm).
**FORMATO**: Markdown, 3 parágrafos, ~400 palavras total.
**LÍNGUA**: Português brasileiro.
**OBJETIVO**: Texto pronto para colunistas COPIAREM e COLAREM.
"""
