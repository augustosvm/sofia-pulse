...
async function main() {
    console.log('🏗️ Creating sofia.tech_metrics table...');

    try {
        await pool.query(`
            CREATE TABLE IF NOT EXISTS sofia.tech_metrics (
            ...
            CREATE INDEX IF NOT EXISTS idx_tech_metrics_lookup ON sofia.tech_metrics(technology, source);
            -- Index for time-range queries
            CREATE INDEX IF NOT EXISTS idx_tech_metrics_time ON sofia.tech_metrics(collected_at);
        `);

        console.log('✅ Table sofia.tech_metrics created/verified.');
