...
class IncrementalCollector:
    ...
    def __enter__(self):
        ...
        try:
            ...
            cursor.execute("SELECT sofia.get_last_successful_run(%s)", (self.collector_name,))
            result = cursor.fetchone()
            self.last_run_time = result[0] if result else None

            # Start new run
            cursor.execute("SELECT sofia.start_collector_run(%s)", (self.collector_name,))
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        try:
            ...
            """,
                (self.run_id, status, self.records_processed, self.records_inserted, self.records_updated, error_msg),
            )
            self.conn.commit()
...
def get_years_to_fetch(
    start_year: int, end_year: int, last_run: Optional[datetime] = None, max_age_days: int = 365
) -> list:
    """
