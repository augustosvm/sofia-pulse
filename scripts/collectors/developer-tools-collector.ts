#!/usr/bin/env tsx
/**
 * Developer Tools Collector - Sofia Pulse
 *
 * Core engine for collecting developer tools data from multiple marketplaces.
 * Tracks tool adoption as a leading indicator for tech trends.
 *
 * Supports:
 * - VS Code Marketplace (extensions)
 * - JetBrains Marketplace (plugins)
 * - Chrome Web Store (dev tools) - disabled until implementation
 *
 * Usage:
 *   npx tsx scripts/collectors/developer-tools-collector.ts vscode-marketplace
 *   npx tsx scripts/collectors/developer-tools-collector.ts jetbrains-marketplace
 *   npx tsx scripts/collectors/developer-tools-collector.ts --all
 */

import { Pool } from 'pg';
import * as dotenv from 'dotenv';
import axios, { AxiosRequestConfig } from 'axios';
import { DeveloperToolsInserter, DeveloperToolData } from '../shared/developer-tools-inserter.js';
...
export interface DeveloperToolCollectorConfig {
  /** Nome único do collector (ex: 'vscode-marketplace') */
  name: string;
  ...
  /** Função que parseia a resposta da API e retorna DeveloperToolData[] */
  parseResponse: (data: any, env: NodeJS.ProcessEnv) => DeveloperToolData[] | Promise<DeveloperToolData[]>;

  /** Schedule cron (ex: '0 14 * * *' for daily at 2pm) */
...
// ============================================================================
// DEVELOPER TOOLS COLLECTOR ENGINE
// ============================================================================

export class DeveloperToolsCollector {
  private pool: Pool;
  private inserter: DeveloperToolsInserter;

  constructor() {
    this.pool = new Pool(dbConfig);
    this.inserter = new DeveloperToolsInserter(this.pool);
  }

  /**
   * Executa um collector baseado em sua configuração
   */
  async collect(config: DeveloperToolCollectorConfig): Promise<{
    success: boolean;
    collected: number;
    errors: number;
    duration: number;
    ...
    try {
      ...
      if (config.requestBody) {
        requestConfig.data = config.requestBody;
      }
      ...
      if (config.rateLimit && typeof config.rateLimit === 'string' && config.rateLimit in rateLimiters) {
        ...
        if (config.method === 'POST') {
          response = await rateLimiter.post(url, config.requestBody, requestConfig);
        } else {
        ...
        if (config.method === 'POST') {
          response = await axios.post(url, config.requestBody, requestConfig);
        } else {
      ...
      const tools = await config.parseResponse(response.data, process.env);
      console.log(`   ✅ Parsed ${tools.length} developer tools`);
      console.log('');
      ...
      for (const tool of tools) {
        try {
          await this.inserter.insertDeveloperTool(tool);
          collected++;
        } catch (error: any) {
          console.error(`   ❌ Error inserting ${tool.tool_name}:`, error.message);
          errors++;
        }
      }

      console.log(`   ✅ Inserted ${collected} developer tools`);
      if (errors > 0) {
      ...
      if (axios.isAxiosError(error)) {
        if (error.response?.status === 403) {
          console.error('   💡 Hint: Check if API key is set or rate limit exceeded');
    ...
    console.log(`   Run ID: ${runId}`);
    console.log(`   Collected: ${collected} developer tools`);
   ...
   */
  async collectAll(configs: DeveloperToolCollectorConfig[]): Promise<void> {
    console.log('');
    console.log('🚀 Running all developer tools collectors...');
    ...
    console.log(`🔧 Total collected: ${totalCollected} developer tools`);
...
export async function runDeveloperToolsCLI(configs: Record<string, DeveloperToolCollectorConfig>) {
  const args = process.argv.slice(2);

  if (args.length === 0 || args.includes('--help') || args.includes('-h')) {
    console.log('');
    console.log('🔧 Developer Tools Collector - Sofia Pulse');
    console.log('');
    console.log('Usage:');
    console.log('  npx tsx scripts/collectors/developer-tools-collector.ts <collector>');
    console.log('  npx tsx scripts/collectors/developer-tools-collector.ts --all');
    console.log('');
  ...
  const collector = new DeveloperToolsCollector();
