...
COUNTRIES = [
    ...
    'PAK', 'BGD', 'LKA', 'NPL', 'MMR', 'KHM', 'LAO', 'MNG', 'HKG', 'TWN',
    # Middle East & Central Asia
    'ARE', 'SAU', 'IRN', 'IRQ', 'ISR', 'TUR', 'KAZ', 'UZB', 'AFG',
    # Americas
    'USA', 'CAN', 'MEX', 'BRA', 'ARG', 'CHL', 'COL', 'PER',
    # Europe
    'DEU', 'FRA', 'GBR', 'ITA', 'ESP', 'RUS', 'POL', 'NLD',
    # Africa
    'ZAF', 'NGA', 'EGY', 'KEN', 'ETH',
    # Oceania
    'AUS', 'NZL',
...
def save_to_database(conn, records: List[Dict], indicator_id: str, indicator_info: Dict, source: str = 'ILO') -> int:
    ...
    for record in records:
        ...
        if 'ref_area' in record:
            # ILO format
            country_code = record.get('ref_area', {}).get('id', '') if isinstance(record.get('ref_area'), dict) else record.get('ref_area', '')
            country_name = record.get('ref_area', {}).get('label', '') if isinstance(record.get('ref_area'), dict) else ''
            sex = record.get('sex', {}).get('id', 'T') if isinstance(record.get('sex'), dict) else record.get('sex', 'T')
