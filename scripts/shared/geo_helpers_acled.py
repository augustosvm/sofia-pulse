...
def normalize_location_acled(conn, country: Optional[str], location: Optional[str]) -> dict:
    ...
    if country and country.strip():
        try:
            with conn.cursor() as cur:
                cur.execute("SELECT get_or_create_country(%s) as country_id", (country.strip(),))
                result = cur.fetchone()
    ...
    if location and location.strip() and country_id:
        try:
            with conn.cursor() as cur:
                cur.execute("SELECT get_or_create_city(%s, NULL, %s) as city_id", (location.strip(), country_id))
                result = cur.fetchone()
