...
def analyze_tables(conn):
    ...
    for table in tables:
        ...
        try:
            ...
            table_stats.append({"name": table_name, "count": count, "has_geo": len(geo_cols) > 0, "geo_cols": geo_cols})
...
def recommend_dashboards(table_stats):
    ...
    [t for t in table_stats if "paper" in t["name"].lower() or "arxiv" in t["name"].lower()]
